...
const scaled = ({ locationX: x, locationY: y }) => {
...
export default class Sketch extends React.Component<Props> {
  ...
  setupPanResponder = () => {
    const onEnd = event => {
      ...
      if (this.props.onChange) {
        this.props.onChange(this.renderer);
      }
      if (this.props.onEndTouch) {
        this.props.onEndTouch();
      }
    };
    ...
    this.panResponder = PanResponder.create({
      ...
      onStartShouldSetPanResponderCapture: () => {
        if (this.props.onStartTouch) {
          this.props.onStartTouch();
        }
        return true;
      },
  ...
  render() {
    return (
      <GLView
        {...(this.props.disabled ? null : this.panResponder.panHandlers)}
